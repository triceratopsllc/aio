...
import type { AuditResult, CodeAuditContext, PageInfo } from '../types.js';
...
function fileToRoute(appRoot: string, file: string): string {
...
function extractBlockKeys(source: string, blockName: string): string[] {
...
function analyzePageSource(source: string): Omit<PageInfo, 'route' | 'file' | 'isDynamic'> {
  ...
  if (hasMetadata) {
    ...
    if (match) {
      ...
      if (end > 0) {
        ...
        const keyRe = /(?:^|[,\s{])([a-zA-Z_][a-zA-Z0-9_]*)\s*:/g;
        let m: RegExpExecArray | null;
        while ((m = keyRe.exec(body))) topLevelKeys.add(m[1]!);
      }
    }
  }
  ...
  let m: RegExpExecArray | null;
  while ((m = typeRe.exec(source))) jsonLdTypes.push(m[1]!);
...
async function collectPages(projectRoot: string): Promise<{ pages: PageInfo[]; rootLayout: string | null }> {
  ...
  if (!appRoot) return { pages: [], rootLayout: null };
  ...
  let rootLayout: string | null = null;
...
export async function runNextjsCodeAudit(opts: { projectRoot: string; name?: string }): Promise<AuditResult> {
  ...
  const mergedPages: PageInfo[] = pages.map((p) => {
    if (!layoutAnalysis) return p;
    const mergeKeys = (a: string[], b: string[]) => [...new Set([...a, ...b])];
    return {
      ...p,
