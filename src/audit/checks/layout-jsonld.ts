...
export const layoutJsonLdCheck: CodeCheck = {
  id: 'layout-jsonld',
  ...
  async run(ctx): Promise<DimensionResult> {
    ...
    if (hasOrg) score += this.maxScore * 0.3;
    if (hasSite) score += this.maxScore * 0.2;
    score = Math.round(score);

    let status: DimensionResult['status'] = 'pass';
    if (!hasOrg || !hasSite) status = 'warn';

    const missing: string[] = [];
    if (!hasOrg) missing.push('Organization');
    if (!hasSite) missing.push('WebSite');
