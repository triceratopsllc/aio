...
export const metadataCheck: CodeCheck = {
  id: 'metadata',
  ...
  async run(ctx): Promise<DimensionResult> {
    ...
    for (const page of pages) {
      ...
      if (!hasAny) {
        ...
        if (missingOg.length) missing.push(`openGraph.${missingOg.join('/')}`);
        if (missingTwitter.length) missing.push(`twitter.${missingTwitter.join('/')}`);
        if (!page.hasAlternatesCanonical) missing.push('alternates.canonical');
        if (missing.length) {
          findings.push({
            severity: 'warn',
    ...
    if (coverageRatio >= 0.9) status = 'pass';
