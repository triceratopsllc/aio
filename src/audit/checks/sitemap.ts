...
export const sitemapCheck: CodeCheck = {
  id: 'sitemap',
  label: 'sitemap.xml',
  maxScore: 10,
  async run(ctx): Promise<DimensionResult> {
    ...
    const sitemapTsExists =
      (await ctx.fileExists('src/app/sitemap.ts')) || (await ctx.fileExists('app/sitemap.ts'));
    const sitemapXmlExists = await ctx.fileExists('public/sitemap.xml');
    const generatorExists =
      (await ctx.fileExists('scripts/generate-sitemap.js')) ||
    ...
    if (urlCount >= 10) score = this.maxScore * 0.8;
    if (referencesDynamic) score = this.maxScore;
    score = Math.round(score);

    let status: DimensionResult['status'] = 'pass';
    if (urlCount < 3 && !referencesDynamic) status = 'warn';
