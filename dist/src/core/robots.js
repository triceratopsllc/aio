...
export function buildRobotsTxt(opts) {
    const { siteUrl, sitemapPath = '/sitemap.xml', allowAllDefault = true, allowAiCrawlers = true, aiCrawlers = [...AI_CRAWLERS], additionalRules = [], disallowPaths = [], extraLines = [], } = opts;
    const blocks = [];
    if (allowAllDefault) {
        blocks.push(renderRule({
            userAgent: '*',
